...
\begin{enumerate}
    \item Realizar un diagnóstico del nivel de digitalización y un levantamiento detallado de requisitos funcionales y no funcionales con las PYMES objetivo, mediante entrevistas con dueños de las PYMES.
    ...
    \item Implementar los módulos de gestión de pedidos (con geolocalización en tiempo real y medios de pago efectivo y transferencia), control de inventarios, administración de repartidores y optimización de rutas, y el dashboard analítico con KPIs.
    
    \item Integrar un chatbot basado en el modelo RAG dentro de la plataforma, accesible por WhatsApp, para automatizar la atención al cliente y el soporte operativo.
...
El proyecto abarca la creación completa de una solución tecnológica operativa, desde su concepción hasta su validación en un entorno controlado. En primer lugar, se realizó un análisis de requisitos basado en un diagnóstico del nivel de digitalización actual de las PYMES objetivo; la recolección de información para los requisitos, el objetivo general y los objetivos específicos se obtuvo mediante entrevistas con dueños de las PYMES, para derivar especificaciones funcionales y no funcionales. El diseño resultante contempla una arquitectura de software modular, compuesta por un backend desarrollado con NestJS que expone una API RESTful, una aplicación móvil multiplataforma construida con React Native (con vistas para clientes, repartidores y cocina), y un sitio web implementado con Next.js que funciona principalmente como panel de administración y además permite iniciar sesión como cliente para realizar compras desde la web. La persistencia de datos se gestionará mediante una base de datos relacional PostgreSQL.

La plataforma integrará módulos funcionales centrales para la gestión operativa: un sistema de pedidos con registro de geolocalización de direcciones y medios de pago únicamente por efectivo y transferencia (sin integración con pasarelas de pago); un módulo de control de inventarios básico; una vista o módulo de \textit{cocina} en la aplicación móvil para aceptar o rechazar pedidos y notificar cuando estén listos para que el repartidor recoja, integrado al flujo de solicitud de órdenes; un panel de administración de repartidores con procesos de aprobación y asignación manual de pedidos; y un dashboard analítico que presente indicadores clave de desempeño (KPIs) derivados de los datos transaccionales. Como componente innovador, se desarrollará e integrará un chatbot basado en el modelo de Generación Aumentada por Recuperación (RAG) a través de WhatsApp, para automatizar respuestas a consultas frecuentes de clientes.
...
Los medios de pago contemplados en la plataforma son únicamente efectivo y transferencia; no se integra servicio de pasarela de pagos. Para garantizar la viabilidad y el enfoque del proyecto, se excluyen además funcionalidades avanzadas y consideraciones de escala industrial. Esto incluye: el desarrollo de una arquitectura multi-tenant; la optimización automática de rutas y el live-tracking de repartidores; las notificaciones en tiempo real (WebSockets/push); y módulos complementarios como marketing, reseñas o gestión de proveedores. Tampoco se contempla el desarrollo de una aplicación nativa para iOS, el despliegue de infraestructura de alta disponibilidad, una validación a escala sectorial, o un plan de soporte y mantenimiento post-implementación.
