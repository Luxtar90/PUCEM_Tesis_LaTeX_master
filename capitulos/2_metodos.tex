...
Se ejecutaron pruebas unitarias en servicios críticos del backend (módulos de autenticación, pedidos, productos, inventarios) utilizando el framework de testing integrado en NestJS (Jest). Las pruebas unitarias validaron la lógica de negocio aislada, como el cálculo de totales de pedidos, la validación de credenciales y la actualización de inventarios. Se alcanzó una cobertura de código del 75\% en los módulos principales.

Las pruebas de integración validaron la comunicación entre componentes: frontend-backend (solicitudes HTTP y respuestas JSON), backend-base de datos (consultas y transacciones), y chatbot-API (consultas a la base de conocimiento). Se verificaron los flujos críticos completos: registro de usuario, creación de pedido, actualización de estados, y consultas al chatbot.
...
\begin{itemize}
  \item \textbf{Disponibilidad del sistema:} 99.2\% durante las pruebas (caídas menores durante mantenimiento programado).
  \item \textbf{Tiempo de respuesta promedio de la API:} 180 ms para endpoints de lectura, 320 ms para operaciones de escritura.
  \item \textbf{Tasa de errores HTTP:} 2.1\% (principalmente errores 400 por validaciones de entrada).
  \item \textbf{Tiempo de carga del panel web:} 1.8 segundos promedio (First Contentful Paint).
  \item \textbf{Uso de recursos del servidor:} CPU promedio 35\%, memoria 60\% durante picos de carga simulada.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Autenticación y autorización (JWT + RBAC):} Tokens generados correctamente, roles aplicados según permisos, acceso denegado a endpoints no autorizados.
  \item \textbf{Gestión de pedidos:} Creación, actualización de estados, asignación a repartidores, y geolocalización funcionando según especificaciones.
  \item \textbf{Control de inventarios:} Descuento automático de stock, alertas de productos agotados, y sincronización con base de datos validadas.
  \item \textbf{Vista de cocina:} Notificaciones de pedidos nuevos, confirmación/rechazo de pedidos, y marcado de pedidos listos operando correctamente.
  \item \textbf{Dashboard analítico:} KPIs calculados correctamente (total de pedidos, ventas del día, tiempo promedio de entrega).
  \item \textbf{Chatbot RAG por WhatsApp:} Respuestas pertinentes a consultas frecuentes, integración con base de conocimiento funcionando, y latencia promedio de respuesta de 3 segundos.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Error en cálculo de totales:} Se corrigió un bug en el cálculo de totales de pedidos cuando se aplicaban descuentos.
  \item \textbf{Pérdida de sesión:} Se ajustó el tiempo de expiración de tokens JWT para evitar cierres de sesión prematuros.
  \item \textbf{Latencia en geolocalización:} Se optimizó la consulta a la API de geolocalización para reducir tiempos de respuesta.
  \item \textbf{Errores de validación en formularios:} Se mejoraron mensajes de error en el frontend para mayor claridad.
\end{itemize}
